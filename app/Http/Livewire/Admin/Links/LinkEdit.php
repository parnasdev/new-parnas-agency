...
class LinkEdit extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.admin.links.link-edit' , compact('categories' , 'link_types' , 'pages'));
    }
    ...
    public function submit()
    {
        ...
        foreach (collect($this->links)->where('parent' , '') as $link) {
            $link['parent'] = null;
            if (isset($link['existInDB']) ? $link['existInDB'] : false) {
            ...
            foreach (collect($this->links)->where('parent' , $link['id']) as $child1) {
                ...
                if (isset($child1['existInDB']) ? $child1['existInDB'] : false) {
                ...
                foreach (collect($this->links)->where('parent' , $child1['id']) as $child2) {
                    ...
                    if (isset($child2['existInDB']) ? $child2['existInDB'] : false) {
