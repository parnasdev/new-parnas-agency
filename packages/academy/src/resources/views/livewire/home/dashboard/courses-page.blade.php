...
{{--                <label for="" class="label-id-courses">{{ $course->id }}</label>--}}
                <a class="w-100" target="_blank" href="{{ $course->post->path() }}">
                    <img class="img-courses" src="{{ $course->post->files->firstWhere('type' , 1)?->url ?? '/images/noPicture.png' }}" alt="">
                </a>
                <div class="details-courses">
                    <div class="info">
                        <span class="title">نام دوره</span>
                        <a href="{{ $course->post->path() }}" target="_blank" class="text">{{ $course->post->title }}</a>
                    </div>
                    <div class="info">
                        <span class="title">نام مدرس</span>
                        <span class="text">{{ \App\Models\User::find($course->post->options['teacher'])?->name . ' ' . \App\Models\User::find($course->post->options['teacher'])?->family }}</span>
                    </div>
                    <div class="info">
                    ...
                    <div class="info">
                        ...
                        <span class="text">{{ number_format($course->post->options['price']) }}</span>
                    </div>
...
@push('title' , 'دوره های خریداری شده')
